...
MODEL=$1
if [[ ${MODEL,,} == *"moe"* ]]; then
  python -m sglang.launch_server --model-path $MODEL --enable-torch-compile --tensor-parallel-size 1 --port $ipnport --disable-cuda-graph & server=localhost:$ipnport/health
else
  python -m sglang.launch_server --model-path $MODEL --enable-torch-compile --tensor-parallel-size 1 --port $ipnport & server=localhost:$ipnport/health
fi
